...
@section('content')
    <div class="card">
        ...
        <form id="debitNoteForm" action="{{ $sale ? route('debit-notes.store', ['sale_id' => $sale->id]) : route('debit-notes.store') }}" method="POST">
            @csrf
            ...
            <div class="card-body">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="company_id" id="company_id" class="form-select" required>
                            ...
                            @foreach($empresas as $empresa)
                                <option value="{{ $empresa->id }}" {{ ($empresaSeleccionada == $empresa->id) ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="typedocument_id" id="typedocument_id" class="form-select" required>
                            ...
                            @foreach($tiposDocumento as $tipo)
                                <option value="{{ $tipo->id }}" {{ ($tipoDocumentoSeleccionado == $tipo->id) ? 'selected' : '' }}>
                                    {{ $tipo->description }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mb-3 row">
                    <div class="col-12">
                        ...
                        <textarea name="motivo" id="motivo" class="form-control" rows="3" placeholder="Describa el motivo de la nota de débito..." required>{{ old('motivo') }}</textarea>
                    </div>
                </div>

                <!-- Historial de Notas de Débito previas -->
                @if($sale && isset($historialNDB) && $historialNDB->count() > 0)
                    <div class="mb-4 card">
                        <div class="card-header">
                            <h6 class="mb-0 card-title">
                                <i class="ti ti-history me-2"></i>
                                Historial de Notas de Débito de esta Factura
                            </h6>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-sm table-bordered">
                                    ...
                                    @foreach($historialNDB as $ndb)
                                        <tr>
                                            <td>{{ $ndb->dte && $ndb->dte->id_doc ? $ndb->dte->id_doc : ('#' . $ndb->id) }}</td>
                                            <td>{{ \Carbon\Carbon::parse($ndb->date)->format('d/m/Y') }}</td>
                                            <td class="text-end">${{ number_format($ndb->totalamount, 2) }}</td>
                                            <td>
                                                @if($ndb->dte && $ndb->dte->estadoHacienda)
                                                    <span class="badge {{ $ndb->dte->estadoHacienda === 'PROCESADO' ? 'bg-success' : ($ndb->dte->estadoHacienda === 'RECHAZADO' ? 'bg-danger' : 'bg-warning') }}">
                                                        {{ $ndb->dte->estadoHacienda }}
                                                    </span>
                                                @else
                                                    <span class="badge bg-secondary">N/A</span>
                                                @endif
                ...
                @if($sale && $sale->salesdetails->count() > 0)
                    <div class="mb-4 card">
                        <div class="card-header">
                            <h6 class="mb-0 card-title">
                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-3 alert alert-info d-flex align-items-center">
                                ...
                                <div>
                                    <strong>Importante:</strong> Las notas de débito solo pueden <strong>AGREGAR</strong> cantidades adicionales.
                                    Seleccione los productos y especifique las cantidades adicionales a cobrar.
                                </div>
                            </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-sm table-bordered">
                                    ...
                                    <tbody>
                                        @foreach($sale->salesdetails as $detalle)
                                            <tr>
                                                ...
                                                <td class="text-end">${{ number_format($detalle->amountp * $detalle->pricesale, 2) }}</td>
                                    ...
                                    <tfoot class="table-light">
                                        <tr>
                                            <th colspan="4" class="text-end">Total Original:</th>
                ...
                <!-- Productos adicionales para la nota de débito -->
                <div class="mb-3 row">
                    <div class="col-12">
                        <h6 class="mb-3">
                            <i class="ti ti-package me-2"></i>
                            Productos Adicionales a Incluir en la Nota de Débito
                        </h6>
                                ...
                                <table class="table table-sm">
                                    <thead>
                                        <tr>
                                            ...
                                            <th width="10%">Cantidad Adicional</th>
                                    ...
                                    <tbody id="productsTable">
                                        @foreach($sale->salesdetails as $index => $detalle)
                                            <tr>
                                                <td>
                                                ...
                                                <td>
                                                    <select name="productos[{{ $index }}][tipo_venta]" class="form-select form-select-sm tipo-venta-select">
                ...
                <div class="row">
                    <div class="col-md-6 offset-md-6">
                        <div class="card bg-light">
                            <div class="card-body">
                                <h6 class="card-title">Resumen de Totales de la Nota de Débito</h6>
                                <div class="row">
...
$(document).ready(function() {
    ...
    $(document).on('change', '.tipo-venta-select', function() {
        const row = $(this).closest('tr');
        calculateRowTotals(row);
        calculateTotals();
    });
    ...
    function calculateRowTotals(row) {
        ...
        const tipoVenta = row.find('.tipo-venta-select').val();
    ...
    $('#debitNoteForm').on('submit', function(e) {
        ...
        if (!hasValidQuantities) {
            ...
            Swal.fire({
                title: 'Error',
                text: 'Debe especificar cantidades adicionales válidas para los productos seleccionados.',
                icon: 'error'
            });
            return false;
        }
