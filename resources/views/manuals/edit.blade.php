...
$(document).ready(function() {
    ...
    const existingContent = document.getElementById('contenido_text').value;
    if (existingContent) {
        ...
        <div class="card">
            <div class="card-header border-bottom">
                <h5 class="mb-3 card-title">
                    <i class="ti ti-edit me-2"></i>Editar Manual
                </h5>
                <p class="text-muted mb-0">Modifique la información del manual de usuario</p>
            </div>
            <div class="card-body">
                <form id="manualForm" action="{{ route('manuals.update', $manual->id) }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select select2" id="modulo" name="modulo" required>
                                    ...
                                    @foreach($modulos as $key => $value)
                                        <option value="{{ $key }}" {{ old('modulo', $manual->modulo) == $key ? 'selected' : '' }}>
                                            {{ $value }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="descripcion" name="descripcion" rows="3"
                                  placeholder="Breve descripción del manual...">{{ old('descripcion', $manual->descripcion) }}</textarea>
                        @error('descripcion')
                            <div class="text-danger small mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea id="contenido_text" name="contenido" style="display: none;">{{ old('contenido', $manual->contenido) }}</textarea>
                        @error('contenido')
                            <div class="text-danger small mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <div class="col-md-3">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="version" name="version"
                                       placeholder="1.0" value="{{ old('version', $manual->version) }}" required>
                                @error('version')
                                    <div class="text-danger small mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-3">
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control" id="orden" name="orden"
                                       placeholder="0" value="{{ old('orden', $manual->orden) }}" min="0" max="999" required>
                                @error('orden')
                                    <div class="text-danger small mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="icono" name="icono"
                                       placeholder="Ej: file-text, book, user" value="{{ old('icono', $manual->icono) }}">
                    ...
                    <div class="mb-3">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" id="activo" name="activo" value="1"
                                   {{ old('activo', $manual->activo) ? 'checked' : '' }}>
                    ...
                    <div class="d-flex justify-content-end gap-2">
                        <a href="{{ route('manuals.show', $manual->id) }}" class="btn btn-label-secondary">
                            <i class="ti ti-eye me-1"></i>Ver Manual
                        </a>
                        <a href="{{ route('manuals.index') }}" class="btn btn-label-secondary">
                            <i class="ti ti-x me-1"></i>Cancelar
                        </a>
                        <button type="submit" class="btn btn-primary">
                            <i class="ti ti-check me-1"></i>Actualizar Manual
                        </button>
                    </div>
                </form>
            </div>
